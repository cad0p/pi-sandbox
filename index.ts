...
export default function (pi: ExtensionAPI) {
  ...
  const PERMISSION_OPTIONS: PromptOption[] = [
    ...
    { label: "Allow for this project", key: "P", action: "project", confirm: true, hint: "→ .pi/sandbox.json" },
    { label: "Allow for all projects", key: "A", action: "global", confirm: true, hint: "→ ~/.pi/agent/sandbox.json" },
  ];
  ...
  async function showPermissionPrompt(
    ...
    const result = await ctx.ui.custom<"abort" | "session" | "project" | "global">((tui, theme, _kb, done) => {
      let selectedIndex = 0;
  ...
  async function promptDomainBlock(
    ...
    return showPermissionPrompt(ctx, `🌐 Network blocked: "${domain}" is not in allowedDomains`, PERMISSION_OPTIONS);
  }
  ...
  async function promptReadBlock(
    ...
    return showPermissionPrompt(ctx, `📖 Read blocked: "${filePath}" is not in allowRead`, PERMISSION_OPTIONS);
  }
  ...
  async function promptWriteBlock(
    ...
    return showPermissionPrompt(ctx, `📝 Write blocked: "${filePath}" is not in allowWrite`, PERMISSION_OPTIONS);
  }
